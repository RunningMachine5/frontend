...
export function RuleManagementPage() {
  ...
  const activationGuide = !draftSet
    ...
    : !validation
      ? "규칙 검증을 먼저 실행하면 활성화 버튼이 열립니다."
      : !validation.valid
        ? "검증 오류를 수정하고 다시 검증하세요."
        : "검증을 통과했습니다. Replay 결과를 확인한 뒤 운영에 반영하세요.";
      ...
      <section className="admin-page rule-admin-page">
        ...
        <nav aria-label="룰 관리 작업" className="rule-workspace-switch" role="tablist">
          ...
          >
            ...
            <em className={validation?.valid ? "complete" : ""}>{validation?.valid ? "검증 통과" : "대기"}</em>
          </button>
        </nav>
          ...
          <section aria-busy={isBusy} aria-labelledby="rule-verify-tab" className="rule-verify-workspace" id="rule-verify-workspace" role="tabpanel">
            <article className="admin-panel verify-control-panel">
              ...
              <div className="verify-actions">
                ...
                <button className="admin-button primary" disabled={!canVerify || !validation?.valid || isBusy} onClick={() => { if (selectedSet && window.confirm(`DRAFT v${selectedSet.version}을 운영 룰셋으로 활성화할까요?`)) void runAction(async () => { await activateRuleSet(selectedSet.id); await refresh(); setWorkspaceTab("edit"); setNotice("DRAFT를 ACTIVE 룰셋으로 반영했습니다."); }); }} type="button">3. DRAFT 활성화</button>
              </div>
              <p className="action-guide"><strong>현재 상태</strong>{activationGuide}</p>
            </article>

            <article className="admin-panel replay-report-panel">
              <div className="panel-title split"><div><p className="admin-eyebrow">IMPACT REPORT</p><h2>ACTIVE 대비 영향 분석</h2><small>유형, 구성요소, 변경 거래 순으로 확인합니다.</small></div>{replay && <em className="report-scope">평가 {replay.evaluated_count.toLocaleString("ko-KR")}건</em>}</div>

              {!replay && <div className="replay-empty"><strong>아직 비교 결과가 없습니다.</strong><p>왼쪽에서 규칙 검증을 통과한 뒤 Replay를 실행하면 운영 룰 대비 변화가 여기에 표시됩니다.</p></div>}

              {replay && <div className="replay-report-content">
                ...
                <section className="report-section"><div className="report-section-title"><div><h3>사기유형별 점수 변화</h3><p>평균 점수와 매칭 거래가 어느 방향으로 움직였는지 비교합니다.</p></div></div><div className="admin-table-wrap"><table className="impact-table"><thead><tr><th>사기유형</th><th>ACTIVE 평균</th><th>DRAFT 평균</th><th>점수 차이</th><th>매칭 거래 차이</th><th>최대 변화</th></tr></thead><tbody>{replay.type_summaries.map((item) => <tr key={item.type_code}><td><strong>{item.display_name}</strong><small>{item.type_code}</small></td><td>{formatScore(item.active_average_score)}</td><td>{formatScore(item.draft_average_score)}</td><td><em className={deltaTone(item.average_score_delta)}>{item.average_score_delta === null ? "—" : formatSigned(item.average_score_delta, 3)}</em></td><td><em className={deltaTone(item.matched_transaction_count_delta)}>{formatSigned(item.matched_transaction_count_delta)}건</em></td><td>{formatScore(item.max_absolute_score_delta)}</td></tr>)}</tbody></table></div></section>

                <section className="report-section"><div className="report-section-title"><div><h3>영향받은 구성요소</h3><p>정의가 바뀌었거나 매칭 건수에 변화가 있는 항목만 표시합니다.</p></div><em>{changedComponents.length}개</em></div>{changedComponents.length === 0 ? <p className="report-empty-row">변화가 감지된 구성요소가 없습니다.</p> : <div className="admin-table-wrap"><table className="impact-table"><thead><tr><th>구성요소</th><th>유형</th><th>가중치</th><th>매칭 차이</th><th>신규 매칭</th><th>매칭 제외</th></tr></thead><tbody>{changedComponents.map((item) => <tr key={`${item.type_code}:${item.component_key}`}><td><strong>{item.display_name}</strong><small>{item.component_key}</small></td><td>{item.type_code}</td><td>{formatScore(item.active_weight)} → {formatScore(item.draft_weight)}</td><td><em className={deltaTone(item.matched_transaction_count_delta)}>{formatSigned(item.matched_transaction_count_delta)}건</em></td><td>{item.newly_matched_transaction_count}건</td><td>{item.no_longer_matched_transaction_count}건</td></tr>)}</tbody></table></div>}</section>

                <section className="report-section"><div className="report-section-title"><div><h3>변경 거래 상세</h3><p>점수 또는 매칭 근거가 달라진 최근 거래입니다.</p></div><em>최대 {replay.detail_limit}건</em></div>{replay.changed_transaction_details.length === 0 ? <p className="report-empty-row">변경된 거래가 없습니다.</p> : <div className="admin-table-wrap"><table className="impact-table transaction-impact-table"><thead><tr><th>거래 ID</th><th>거래 시각</th><th>변경 유형</th><th>최대 점수 변화</th><th>유형별 점수 차이</th></tr></thead><tbody>{replay.changed_transaction_details.map((item) => <tr key={item.transaction_id}><td>#{item.transaction_id}</td><td>{formatDateTime(item.transaction_datetime)}</td><td>{item.score_changed && item.evidence_changed ? "점수·근거" : item.score_changed ? "점수" : "근거"}</td><td>{item.max_absolute_score_delta.toFixed(3)}</td><td className="transaction-delta">{transactionDeltaSummary(item.score_deltas)}</td></tr>)}</tbody></table></div>}{(replay.has_more || replay.changed_details_truncated) && <p className="report-footnote">표본 또는 상세 표시 한도를 초과한 거래가 있습니다. 더 큰 표본으로 다시 실행할 수 있습니다.</p>}</section>

                {replay.error_details.length > 0 && <section className="report-section error-report"><div className="report-section-title"><div><h3>평가 오류</h3><p>Replay 중 계산하지 못한 거래입니다.</p></div><em>{replay.error_count}건</em></div><ul>{replay.error_details.map((item) => <li key={item.transaction_id}><strong>#{item.transaction_id}</strong><span>{item.error}</span></li>)}</ul></section>}
              </div>}
            </article>
          </section>
        )}
