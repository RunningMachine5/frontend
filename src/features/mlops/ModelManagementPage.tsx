      ...
      <section className="admin-page">
        ...
        {token && <>
          <section className="admin-metrics">
            ...
            <article><span>Serving 상태</span><strong className={!serving?.reconciling ? "positive" : "accent"}>{serving ? `${serving.reconciling ? "전환 중" : "정상"} · ${trafficPercent}%` : "확인 불가"}</strong><small>{serving?.latest_ready_revision ?? "Ready revision 없음"}</small></article>
          </section>
          ...
          <section className="model-workspace">
            <article className="admin-panel runs-panel">
              <div className="panel-title split"><div><p className="admin-eyebrow">TRAINING RUNS</p><h2>학습 실행 이력</h2><small>행을 선택하면 MLflow 원본 지표와 배포 상태를 조회합니다.</small></div><button className="admin-button compact" disabled={isBusy} onClick={() => void refresh()} type="button">상태 새로고침</button></div>
