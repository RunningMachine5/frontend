...
- **버튼 라벨(3.3).** PRD 표기로 구현했다. 디자인 원본 라벨을 쓰려면 `messages.md`에
  먼저 추가해야 한다.
- **고령자 UI.** 응답의 `is_older`가 참이면 챗봇 화면의 글씨·여백·아바타·버튼·입력창을
  한 단계 키운다([chatbotSizes.ts](../src/features/chatbot/chatbotSizes.ts)의 두 벌 값).
  디자인 원본에 있던 "큰 글씨로 보기" 토글은 넣지 않았다 — 나이로만 갈린다.
  본인인증 화면은 아직 `is_older`를 모르는 시점이라 기본 크기 하나로 그린다.
